...
音频文件元数据（Metadata）信息补全、批量格式转换工具。
...
* 为何要做这样一个工具？
    ...
    * 但是：
        ...
        * 好不容易啥信息都显示的很全，而偏偏`专辑封面模糊`的像一坨屎，瞬间没了欣赏音乐的心情。
* 目前实现的功能：
    * 音频文件内的`元数据信息填充补全`。
    ...
    * 目前支持`MP3`、`Ogg Vorbis`以及`FLAC`音频文件的元数据信息补全。
    * 目前支持`FLAC`格式批量转换为`Ogg Vorbis`格式（多线程并行转换）。
* 音频文件元数据的获取：
    * 是基于第三方在线音乐服务提供商所暴露的API接口实现的。
* 音频文件的格式转换：
    * 是基于已编译好的`多平台FFmpeg`+`FFmpeg For Java Wrapper组件`实现的。

## 快速开始

* 使用本工具，有两种途径，分别是`运行已编译好的Jar包`和`自行编译Jar包并运行`。
* 工具基于Java开发。
    - 要运行工具，请安装并配置`Java（v1.8或更高版本）`环境变量。
    - 要自行编译并运行工具，请安装并配置`Java（v1.8或更高版本）`及`Gradle（v7.0或更高版本）`环境变量。

### 使用已编译好的Jar包【推荐大部分用户使用】
...
* 基于的默认配置：
    ...
    * 转换格式后的音频文件存放目录: < 使用前，请先根据功能编号，自定义转换格式后的音频文件存放目录！>
    * songTitleFilter:        已开启
    ...
    * 当前已选择的音频文件格式转换器：< 未选择音频文件格式转换器！请输入对应功能编号来设置！ >
* 快速上手指南：
    ...
    6. 双击`run.bat`（for Microsoft Windows）/`run.sh`（for *Unix），来启动工具。
        * **音频元数据信息补全的操作流程**
            1. 根据提示，输入序号1，`设置原始音频文件的存放目录`。将目录位置路径拖动或粘贴到窗口中，并按回车键确认。
            2. 根据提示，输入序号2，`分析`音频文件元数据信息。
            3. 分析完成后会显示有多少个文件需要补全。根据提示，输入Y或y，补全音频文件元数据信息。
        * **音频文件格式转换的操作流程**
            1. 根据提示，输入序号1，`设置原始音频文件的存放目录`。将目录位置路径拖动或粘贴到窗口中，并按回车键确认。
            2. 根据提示，输入序号3，`自定义转换格式后的音频文件的存放目录`。将目录位置路径拖动或粘贴到窗口中，并按回车键确认。
            3. 根据提示，输入序号4，从屏幕显示的`受支持的音频格式转换器列表`中，找到所需音频文件转换器名称前的序号，将此序号输入到窗口中，并按回车键确认。
            4. 根据提示，输入序号5，开始进行音频文件批量转换。
