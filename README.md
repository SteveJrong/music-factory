...
* 快速上手指南：
    ...
    3. 打开命令提示符（for Microsoft Windows）/终端（for *Unix）窗口，切换到项目`distribution`目录下。
    4. 执行`java -jar music-factory-XXX-XXXXXX-all.jar` 命令，或双击`run.bat`（for Microsoft Windows）/`run.sh`（for *Unix），启动工具。
...
* 快速上手指南：
    1. 将要进行元数据信息补全的音频文件，复制一份到别处作为`备份`，以免数据损坏无法恢复。
    2. 在电脑任意位置`新建`一个`空文件夹`，将要进行元数据信息补全的音频文件拖入其内。
    3. 打开命令提示符（for Microsoft Windows）/终端（for *Unix）窗口，切换到项目的根目录目录下。
    ...
    7. 执行`java -jar music-factory-XXX-XXXXXX-all.jar` 命令，或双击`run.bat`（for Microsoft Windows）/`run.sh`（for *Unix），启动工具。
